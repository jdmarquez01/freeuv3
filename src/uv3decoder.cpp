...
private:
    ...
    std::istream& serialize(std::istream& is) {
        is.read((char*)&l_, sizeof(l_)); /* Flawfinder: ignore */
        serialize_impl(is);
        if(!is.good())
            assert(false);
        return is; 
...
struct UV31 : UV3Node
{
    ...
    std::istream& serialize_impl(std::istream& is) override {
        is.read((char*)&kk, sizeof(kk)); /* Flawfinder: ignore */
        return is;
    }
};
...
struct UV32 : UV3Node
{
    struct tmp
    {
        tmp() : reservedSize{0},unknownStuff{},realSizeOfTheNode{0},kk{0},isDirectory{0},kk2{0},kkk{0},kf{0},indexStruct34{0},kh{0},unknownEnumerated{0} {} 
        int reservedSize;
...
struct UV33 : UV3Node
{
    ...
    virtual std::istream& serialize_impl(std::istream& is) override {
        internal n;
        is.read((char*)&n, sizeof(n)); /* Flawfinder: ignore */
...
private:
    struct internal
    {
        internal() : indexNodeMaybe_{0},unknownField_{0},isDirectory_{0},size_{0},zero_{0},name_{0} {}
        int      indexNodeMaybe_;
        int      unknownField_;
...
class UV3FileDer : public UV3File
{
    ...
    public:
        ...
        public:
            explicit UV34StreamBuf(std::shared_ptr<std::ifstream>& fptr, std::vector<UV34*>& nodes,
                                   size_t total_size, std::size_t buff_sz = 0x8000 - 32,
                                   std::size_t put_back = 0)
                    : nodes_(nodes)
        ...
        private:
            ...
            int_type underflow() override {
                ...
                fptr_->read(start, std::min(buffer_.size() - (start - base + put_back_),total_size_ - current_pos_)); /* Flawfinder: ignore */
                if(fptr_) {
                    n = fptr_->gcount();
                }
                current_pos_ += n;
...
public:
    ~UV3FileDer() { assert(false); }
    UV3FileDer(std::shared_ptr<std::ifstream>& st, UV32* n, const std::string& s,
               const std::string& path)
            : UV3File{stream, s, path}
            , stream(nullptr) {
        // filename_ = s;
        // path_ = path;
        ...
        if(n->kk.realSizeOfTheNode < 0x8000 - 96) {
            ...
            st->read(c.data(), n->kk.realSizeOfTheNode); /* Flawfinder: ignore */
...
void UV3Decoder::open(const char* filename) {
    ...
    for(int i = 2; !d_ptr->ifs->eof(); ++i) {
        ...
        if(l != nullptr) {
            ...
            if(l->ID == 33)
                p33.push_back(dynamic_cast<UV33*>(l.get()));
        }
    ...
    for(auto n : p33) {
        for(auto i = 0U; i < n->data_.size(); ++i) {
            ...
            {
                auto kk = new UV3FileDer(d_ptr->ifs, n->data_[i],
                                         std::string(n->names_[i].begin(), n->names_[i].end()),
 ...
 */ 
UV3Decoder::UV3Decoder(const char* filename)
        : d_ptr{new UV3DecoderPrivate} {
    if(filename != nullptr)
        open(filename);
}
/**
 ...
 * @return false
 */
bool UV3Decoder::is_open() const { return d_ptr->ifs->is_open(); }
 ...
 * @return false
 */
bool UV3Decoder::good() const { return d_ptr->good; }
